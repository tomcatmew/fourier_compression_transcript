...
def compress_svd():
    ...
    with open('result/{}_svd.csv'.format(file_name), 'w'):
        pass
    print("path:", bvh_path)
    print("scale:", scale)
    ...
    U, s, Vh = numpy.linalg.svd(np_trg, full_matrices=True)
    assert s.shape[0] == 133
    nframe = U.shape[0]
    for nmode in range(1,30):
        s0 = s.copy()
        s0[nmode:] = 0.
        S = numpy.zeros((nframe, 133), dtype=U.dtype)
        S[:133, :133] = numpy.diag(s0)
        np_out = numpy.dot( numpy.dot(U, S), Vh)
        np_bvh1 = bvh_util.decode_continuous_to_bvh(np_out.astype(numpy.float64))
        set_parameter_history_bvh_double(bvh, np_bvh1)
        # bvh.save("result/{}_{}.bvh".format(file_name, itr))
        frame_jp1 = get_joint_position_history_bvh(bvh)
        jnt_diff_ratio = (frame_jp0 - frame_jp1).max() / scale
        num_param = (nframe + 133 + 1) * nmode
        comp_ratio = np_bvh.size / num_param
        print(nmode,comp_ratio,jnt_diff_ratio)
        with open('result/{}_svd.csv'.format(file_name), 'a') as f:
            f.write("{},{},{}\n".format(nmode, comp_ratio, jnt_diff_ratio))
